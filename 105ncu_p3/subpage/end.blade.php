<?php
$table_prefix = 'ncu.dbo.ncu_102_p3';
...
$friends = DB::table('rows.dbo.row_20170626_145821_l8eby AS u')
	->leftJoin('rows.dbo.row_20170626_145821_l8eby AS um', function($join){
		$join->on('u.C3384', '=', 'um.C3384');
	})
	->leftJoin($table_prefix . '_network AS n', 'u.id', '=', 'n.newcid')
	->whereNull('n.complete')
	->where('um.id', $newcid)->select('u.C3386 AS stdname','u.id')->get();
...
$orders = DB::table($table_prefix . '_network AS n1')
	->leftJoin('rows.dbo.row_20170626_145821_l8eby AS u', 'n1.newcid', '=', 'u.id')
	->where('n1.complete', true)
	->orderBy('n1.completed_at', 'ASC')
	->select('n1.id', 'n1.completed_at', 'u.C3384 AS dep', 'u.C3386 AS stdname')
	->distinct()
	->take(100)->get();
	...
	<div class="ui middle aligned stackable grid container">
		<div class="row">
			...
			<div class="five wide column">
				...
				<p><span style="color:#F00"><?=secure_url('', $parameters = array('105ncu_p3')) . '?id=' . $network->id?></span></p>
			</div>
		</div>
	</div>
...
@include('ques.data.105ncu_p3.footer')