<?php
$table_prefix = 'ncu.dbo.ncu_104_p1';
...
$friends = DB::table('rows.dbo.row_20170626_144347_irgy5 AS u')
	->leftJoin('rows.dbo.row_20170626_144347_irgy5 AS um', function($join){
		$join->on('u.C3346', '=', 'um.C3346');
	})
	->leftJoin($table_prefix . '_network AS n', 'u.id', '=', 'n.newcid')
	->whereNull('n.complete')
	->where('um.id', $newcid)->select('u.C3347 AS stdname','u.id')->get();
...
$orders = DB::table($table_prefix . '_network AS n1')
	->leftJoin('rows.dbo.row_20170626_144347_irgy5 AS u', 'n1.newcid', '=', 'u.id')
	->where('n1.complete', true)
	->orderBy('n1.completed_at', 'ASC')
	->select('n1.id', 'n1.completed_at', 'u.C3346 AS dep', 'u.C3347 AS stdname')
	->distinct()
	->take(100)->get();
	...
	<div class="ui middle aligned stackable grid container">
		<div class="row">
			...
			<div class="five wide column">
				...
				<p><span style="color:#F00"><?=secure_url('', $parameters = array('105ncu_p1')) . '?id=' . $network->id?></span></p>
			</div>
		</div>
	</div>
...
@include('ques.data.105ncu_p1.footer')