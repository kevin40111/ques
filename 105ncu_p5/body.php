...
app.controller('loginController', function($scope, $http, $location, CSRF_TOKEN, $q, $mdToast) {
    ...
    $scope.getDepartments = function() {
        $scope.loading = true;
        $http({method: 'POST', url: '105ncu_p5/public/departments', data:{}})
    ...
    $scope.login = function() {
        if (!$scope.dep_name) {
            ...
            $http({method: 'POST', url: '/ques/<?=$doc->dir?>/qlogin', data:{_token: CSRF_TOKEN, dep_name: $scope.dep_name, stu_id: $scope.stu_id, recommend: $scope.recommend}})
            .success(function(data, status, headers, config) {
                if (data.errors) {
