<?php
$table_prefix = 'ncu.dbo.ncu_100_p5';
...
$friends = DB::table('rows.dbo.row_20170626_153718_rkxak AS u')
	->leftJoin('rows.dbo.row_20170626_153718_rkxak AS um', function($join){
		$join->on('u.C3393', '=', 'um.C3393');
	})
	->leftJoin($table_prefix . '_network AS n', 'u.id', '=', 'n.newcid')
	->whereNull('n.complete')
	->where('um.id', $newcid)->select('u.C3395 AS stdname','u.id')->get();
...
$orders = DB::table($table_prefix . '_network AS n1')
	->leftJoin('rows.dbo.row_20170626_153718_rkxak AS u', 'n1.newcid', '=', 'u.id')
	->where('n1.complete', true)
	->orderBy('n1.completed_at', 'ASC')
	->select('n1.id', 'n1.completed_at', 'u.C3393 AS dep', 'u.C3395 AS stdname')
	->distinct()
	->take(100)->get();
	...
	<div class="ui middle aligned stackable grid container">
		<div class="row">
			...
			<div class="five wide column">
				...
				<p><span style="color:#F00"><?=secure_url('', $parameters = array('105ncu_p5')) . '?id=' . $network->id?></span></p>
			</div>
		</div>
	</div>
