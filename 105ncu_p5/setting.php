...
return array(
    ...
    'logInput' => false,
    'logInputDir' => '//192.168.0.125/quesnlb_ap/WEB_log/QUES-DB/105ncu_p5',

    'auth' => array(
        'loginView' => array(
            'intro' => 'ques.data.105ncu_p5.intro',
            'head' => 'ques.data.105ncu_p5.head',
            'body' => 'ques.data.105ncu_p5.body',
            'footer' => 'ques.data.105ncu_p5.footer'
        ),
        'endView' => 'ques.data.105ncu_p5.end',
        'testID' => 'A228909170',
        'primaryID' => 'newcid',
        'input_rull' => array(
            'dep_name' => 'required',
        ...
        'checker' => function(&$validator,$controller){
            ...
            $user_table = DB::table('rows.dbo.row_20170626_153718_rkxak')->where('C3393', $dep_name)->where('C3396', $stdidnumber)->select('id');

            if ($user_table->exists()) {
                $user = $user_table->first();
                Ques\Answerer::login('105ncu_p5', $user->id);
                if (!DB::table('ncu.dbo.ncu_100_p5_network')->where('newcid', $user->id)->exists()) {
                    $commend = DB::table('ncu.dbo.ncu_100_p5_network')->where('id', Input::get('recommend'))->select('newcid')->first();
                    $newcid_commend = isset($commend) ? $commend->newcid : 0;
                    DB::table('ncu.dbo.ncu_100_p5_network')->insert([
                        'newcid' => $user->id,
    ...
    'hide' => function($page){
        if( $page == '7' ) {
            $page1 = DB::table('ncu.dbo.ncu_100_p5_page2')->where('newcid', Ques\Answerer::newcid())->select('p2q1')->first();
            if ($page1->p2q1 == '3' || $page1->p2q1 == '4') {
                return ['QID_vxdg4cht','QID_sdewaw5r'];
            }
        }
    },
