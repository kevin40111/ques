...
return array(
    'logInput' => true,
    'logInputDir' => '//192.168.0.125/quesnlb_ap/WEB_log/QUES-DB/u0016phd',
    'skip' => false,

    'auth' => array(
        'loginView' => array(
            'intro' => 'ques.data.u0016ma.intro',
            'head' => 'ques.data.u0016phd.head',
            'body' => 'ques.data.u0016phd.body',
            'footer' => 'ques.data.u0016phd.footer'
        ),
        'endView' => 'ques.data.u0016phd.end',
        'primaryID' => 'newcid',
        'input_rull' => array(
        ...
        'checker' => function(&$validator, $controller){
            ...
            $user_table = DB::table('rows.dbo.row_20160128_155439_s6wq7')->where('C549', '博士班')->where('C547', $stdnumber)->where('C550', $stdidnumber_last5);

            if ($user_table->exists()) {
                Ques\Answerer::login('u0016phd', $user_table->first()->id);
            }
            else
            {
