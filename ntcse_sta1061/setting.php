...
return array(
	'logInput' => true,
	'logInputDir' => '//192.168.0.125/quesnlb_ap/WEB_log/QUES-DB/1061_ntcse_sta',
    'login_customize' => 1,
 	'skip' => false,
	'auth' => array(
		'loginView' => array(
			'intro' => 'ques.data.ntcse_sta1061.intro',
			'head' => 'ques.data.ntcse_sta1061.head',
			'body' => 'ques.data.ntcse_sta1061.body',
			'footer' => 'ques.data.ntcse_sta1061.footer'
		),
		'endView' => 'ques.data.ntcse_sta1061.end',
		'testID' => 'A228909170',
		'primaryID' => 'newcid',
		'input_rull' => array(
		...
		'checker' => function(&$validator,$controller){
			$user_table = DB::table('ntcse106_1.dbo.ntcse106sta_pstat')->where('newcid',Input::get('identity_id'));
			if( $user_table->exists() ){
				$user = $user_table->select('newcid','ques','sch_id')->first();

                Ques\Answerer::login('ntcse_sta1061', $user->newcid);
