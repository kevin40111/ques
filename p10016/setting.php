...
return array(
    ...
    'auth' => array(
        ...
        'checker' => function(&$validator, $controller) {
            ...
            $user_table = DB::table('rows.dbo.row_20160429_153119_mbwud')->where('C1082', $udepname)->where('C1084', $stdidnumber_last5)->select('id');

            if ($user_table->exists()) {
                $user = $user_table->first();
                Answerer::login('p10016', $user->id);
                if (!DB::table('tiped_104_0016.dbo.tiped_104_0016_p1_network')->where('newcid', $user->id)->exists()) {
                    $commend = DB::table('tiped_104_0016.dbo.tiped_104_0016_p1_network')->where('id', Input::get('id'))->select('newcid')->first();
                    $newcid_commend = isset($commend) ? $commend->newcid : 0;
                    DB::table('tiped_104_0016.dbo.tiped_104_0016_p1_network')->insert([
                        'newcid' => $user->id,
